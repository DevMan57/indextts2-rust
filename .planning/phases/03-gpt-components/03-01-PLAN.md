...
must_haves:
  truths:
    - "Conformer attention uses learned per-head position biases (simplified relative position)"
    - "pos_bias_u and pos_bias_v fields exist and are loaded from checkpoint when available"
    - "linear_pos projection exists and is loaded from checkpoint when available"
    - "Fallback warnings appear ONLY if checkpoint genuinely lacks these tensors"
  artifacts:
...
Note: This plan implements a simplified version that adds learned per-head biases to the query. Full Shaw-style relative position attention would require position encodings passed to forward(), which is a larger architectural change reserved for future optimization.
</objective>
...
Print Conformer output tensor statistics (mean, std) to verify non-random behavior - follow Phase 2 pattern.
  </verify>
  <done>Conformer attention applies pos_bias_u/v to queries before attention computation. Output statistics verify position biases function correctly.</done>
</task>

<task type="auto">
  <name>Task 3: Add unit tests and verify checkpoint tensor loading</name>
  <files>src/models/gpt/conformer.rs</files>
  <action>
...
Run inference with real checkpoint and verify no fallback warnings:
```bash
cargo run --release --bin indextts2 -- --cpu infer \
  --text "Test" --speaker speaker_16k.wav --output /dev/null 2>&1 | \
  grep -E "Missing tensor.*(linear_pos|pos_bias)"
```

If output is empty, checkpoint has these tensors and they load correctly.
If output shows warnings, document which tensors are missing (may be expected if checkpoint variant differs).
  </action>
  <verify>
`cargo test conformer` passes with all tests green.
Run inference command above and confirm either:
  - No fallback warnings (checkpoint has tensors) OR
  - Documented which tensors are missing from this checkpoint variant
  </verify>
  <done>Unit tests verify relative position components load and compute correctly. Checkpoint tensor presence verified.</done>
</task>
...
3. Run inference and grep output for "Missing tensor.*(linear_pos|pos_bias)" - document results
4. `grep -n "pos_bias_u" src/models/gpt/conformer.rs` shows field definition and usage
5. Verify pos_bias fields are used in forward() computation (not just stored)
</verification>

<success_criteria>
- MultiHeadAttention has linear_pos, pos_bias_u, pos_bias_v fields
- Fields are loaded from gpt.safetensors checkpoint (or fallback with warning if absent)
- Forward pass incorporates position biases via query addition
- All existing tests pass plus new tests for position components
- Checkpoint tensor loading verified with real inference run
</success_criteria>
