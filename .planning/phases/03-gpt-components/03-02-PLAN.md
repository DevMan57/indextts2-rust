...
must_haves:
  truths:
    ...
    - "No 'using random weights' warnings for Perceiver components"
  artifacts:
  ...
  key_links:
    - from: "PerceiverResampler"
      to: "gpt.safetensors"
      via: "load proj_context, SwiGLU FFN weights [3412, 1280] and [1280, 1706]"
...
<task type="auto">
  <name>Task 1: Add proj_context and fix CrossAttention dimensions</name>
...
**Add proj_context field to PerceiverResampler:**
```rust
pub struct PerceiverResampler {
    // ... existing fields ...
    /// Context projection: 512 -> 1280
    proj_context: Option<Linear>,
    /// Final norm (gamma only)
...
Store attn_dim as well since it differs from latent_dim.
  </action>
  <verify>
`cargo build` succeeds with no errors.
Grep for "proj_context" in perceiver.rs.
  </verify>
  <done>PerceiverResampler has proj_context field that projects 512-dim context to 1280-dim before attention.</done>
</task>
...
<task type="auto">
  <name>Task 3: Update forward pass to apply proj_context and final_norm</name>
  <files>src/models/gpt/perceiver.rs</files>
  <action>
...
Also update PerceiverConfig to reflect the actual dimensions:
```rust
impl Default for PerceiverConfig {
    fn default() -> Self {
        Self {
            dim: 1280,           // Latent dimension (updated from 512)
            context_dim: 512,    // Input context dimension from Conformer
            num_latents: 32,
            num_heads: 8,
            num_layers: 2,
            ff_mult: 4,          // Not used with SwiGLU but keep for API compat
        }
    }
}
```

Add context_dim field to PerceiverConfig if needed for clarity.
  </action>
  <verify>
`cargo build` succeeds.
`cargo test perceiver` passes.
Run inference and verify Perceiver produces non-zero output with correct shape.
  ...
  <done>Perceiver forward pass applies proj_context before attention and final_norm after layers.</done>
</task>
...
3. Run inference and grep for "Missing tensor.*perceiver" - should see NO warnings for main components
...
- CrossAttention handles asymmetric dims correctly
